...
EXTERN_C __declspec(dllexport) int __stdcall Blend_MMX(MyImage* original, MyImage* blendTo, MyImage* finalImage, float blend_factor) {
	...
	for (int y = 0; y < blendTo->height; ++y)
	{
		...
		{
			//FIRST  PIXEL!
			__m64 src_data = *reinterpret_cast<__m64*>(src); //src_data = s(RGBA RGBA)
			__m64 blend_data = *reinterpret_cast<__m64*>(blend); //blend_data = b(RGBA RGBA)

			__m64 src_unpacked = _mm_unpacklo_pi8(src_data, zero); //src_unpacked = s(0R 0G 0B 0A)
			__m64 blend_unpacked = _mm_unpacklo_pi8(blend_data, zero); //blend_unpacked = b(0R 0G 0B 0A)
			src_unpacked = _mm_sub_pi16(src_unpacked, blend_unpacked); //src_unpacked = s-b(RR GG BB AA)
			blend_unpacked = _mm_slli_pi16(blend_unpacked, 8); //b(R0 G0 B0 A0)
			src_unpacked = _mm_mullo_pi16(src_unpacked, alpha);
			__m64 result_first = _mm_add_pi16(blend_unpacked, src_unpacked); //result = RR GG BB AA
			result_first = _mm_srli_pi16(result_first, 8);//result = 0R 0G 0B 0A
			//SECOND PIXEL!
			src_unpacked = _mm_unpackhi_pi8(src_data, zero); //src_unpacked = s(0R 0G 0B 0A)
			blend_unpacked = _mm_unpackhi_pi8(blend_data, zero); //blend_unpacked = b(0R 0G 0B 0AA)
			src_unpacked = _mm_sub_pi16(src_unpacked, blend_unpacked); //src_unpacked = s-b(RR GG BB AA)
			blend_unpacked = _mm_slli_pi16(blend_unpacked, 8); //b(R0 G0 B0 A0)
			src_unpacked = _mm_mullo_pi16(src_unpacked, alpha);
			__m64 result_second = _mm_add_pi16(blend_unpacked, src_unpacked); //result = RR GG BB AA
			result_second = _mm_srli_pi16(result_second, 8); //result = 0R 0G 0B 0A
			//SECOND PIXEL!
			__m64 big_result = _m_packuswb(result_first, result_second);
			*reinterpret_cast<__m64*>(dst) = big_result;
			dst += 8; //2 pixels at a time.
			src += 8; //2 pixels at a time.
			blend += 8; //2 pixels at a time.
		}
	}
	//_mm_empty();
	QueryPerformanceCounter(&after);
